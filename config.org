...
#+end_src
** Fonts
Um den richtigen Namen einer Schriftart zu finden, verwende =M-x describe-font=.
...
** Formatter
*** TODO void variable
~(void-variable +format-on-save-disabled-modes)~
...
#+RESULTS:
| nxml-mode | sql-mode | tex-mode | latex-mode | LaTeX-mode | org-msg-edit-mode |
...
#+end_src
** Sortierung
Bei =sort-lines= und anderen Sortierfunktionen will ich, dass die Groß-/Kleinschreibung ignoriert wird.
...
#+end_src
*** Test runner
Für die Tests mit =jest= habe ich in =packages.el= das Package =jest-test-mode= hinzugefügt.

Github: https://github.com/rymndhng/jest-test-mode

Damit der =jest-test-mode= ist ein minor mode. Damit er immer aktiviert wird, wenn ich TS bearbeite, braucht es diesen hook:

#+begin_src emacs-lisp
(add-hook 'typescript-mode-hook 'jest-test-mode)
#+end_src

Und ein paar Keybindings braucht es auch noch:
...
Das popup soll automatisch ausgewählt sein, primär, um es mit =q= gleich wieder schließen zu können.
#+begin_src emacs-lisp
...
#+END_SRC
***** Insert datetree-heading
#+name: my/org-insert-datetree-heading
#+begin_src emacs-lisp :tangle no
...
(after! org
  ...
  <<ox-twiki>>
  <<ox-gfm>>
  <<my/org-insert-datetree-heading>>
...
: racket
*** Formatter
Offenbar funktioniert die default Zeilenbreite nicht, daher:

#+begin_src emacs-lisp :tangle no
(after! racket-mode (add-to-list 'apheleia-formatters '(raco-fmt "raco" "fmt" "--width" "80")))
#+end_src
...
*** tidy
Die Einrückungen in der Standardeinstellung funktionieren wirklich (alles unter ~<body>~ ist auf einer Ebene).
...
#+RESULTS: run-xspec-on-file
: run-xspec-on-file

#+name: rerun-last-xspec-test
#+begin_src elisp
...
#+RESULTS: rerun-last-xspec-test
: rerun-last-xspec-test

#+name: browse-xspec-report-url
#+begin_src elisp :tangle no
...
#+RESULTS: mu4e-maildir-shortcuts
: ((/work/Inbox . 105) (/schuhu/INBOX . 115) (/gmail/INBOX . 103) (/schuhu_mjn/INBOX . 109) (/stefanschuh/INBOX . 112))
...
#+RESULTS: mail-accounts
: #s(mu4e-context "gmail" #[nil ((mu4e-message "Switched to %s" label)) ((label . "gmail"))] #[nil ((setq +mu4e-personal-addresses nil) (if (fboundp 'mu4e-clear-caches) (mu4e-clear-caches))) (t)] #[(msg) ((if msg (progn (string-prefix-p (format "/%s" label) (mu4e-message-field msg :maildir) t)))) ((label . "gmail"))] ((user-mail-address . "stefan.schuh.ba@gmail.com") (user-full-name . "Stefan Schuh") (mu4e-drafts-folder . "/gmail/drafts") (mu4e-sent-folder . "/gmail/sent") (mu4e-trash-folder . "/gmail/trash")))
...
*** sent-mail-behaviour
#+name: mu4e-sent-messages-behavior
#+begin_src emacs-lisp
(setq mu4e-sent-messages-behavior 'sent)
#+end_src

Das folgende funktioniert nicht, warum auch immer:
#+begin_src emacs-lisp :tangle no
(setq mu4e-sent-messages-behavior (lambda ()
                                    (if (string= (message-sendmail-envelope-from) "stefan.schuh.ba@gmail.com")
       'delete 'sent)))
#+end_src

#+RESULTS: mu4e-sent-messages-behavior
| lambda | nil | (if (string= (message-sendmail-envelope-from) stefan.schuh.ba@gmail.com) 'delete 'sent) |
...
Und für die Symbole im citar-ui:
#+begin_src elisp
...
** magit
*** forge
Um auf gitlab-issues etc. zuzugreifen braucht es ein bisschen config. Doku siehe hier: https://docs.magit.vc/forge/Setup-for-Another-Gitlab-Instance.html

Zuallererst muss ~(git +forge)~ in ~init.el~ gesetzt sein.

Dann muss ~git~ richtig konfiguriert werden, damit der user auch vorhanden ist:

#+begin_src shell :tangle no
git config --global gitlab.gitlab.obvsg.at/api/v4.user ss
#+end_src
...
#+end_src
** mrk2xml
*** *OBSOLET* Externes Python-Script

#+begin_quote
Diese Funktionalitäten werden von [[https://github.com/schuach/mrc.el][mrc.el]] bereitgestellt. Das Python-Script gibt ist inzwischen auch verloren gegangen.

Die Umwandlung des Basisformats in XML fehlt noch. Aber das mache ich, wenn es wieder mal gebraucht wird.
#+end_quote
